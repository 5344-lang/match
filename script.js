const firebaseConfig = {
  apiKey: "AIzaSyCuEtSNdOxwoGKdaibeFhRxW8OKbCyfKtM",
  authDomain: "secretmatch-app.firebaseapp.com",
  projectId: "secretmatch-app",
  storageBucket: "secretmatch-app.firebasestorage.app",
  messagingSenderId: "92128261613",
  appId: "1:92128261613:web:197ea5fcfdd5e1a7d723e4"
};
...
function updateTickerDisplay(data) {
  ...
  else if (data?.isMatchingActive) autoMsg = `💘 ${title} 진행 중입니다! 카드를 살펴보고 지망을 제출해주세요 ✨${statsStr}`;
  else if (data?.isProfileCheckActive) autoMsg = `🔍 ${title} 준비 중 · 내 프로필을 점검하고 참여 여부를 확정해주세요 · 곧 매칭이 시작됩니다!${statsStr}`;
  else if (data?.adminStep != null) autoMsg = `⏳ ${title} · 참여 여부를 지금 정해주세요!${statsStr}`;
...
function updateParticipantCountUI(snap) {
  let participating = 0, submitted = 0, matched = 0;
  snap.forEach(doc => {
    const u = doc.data();
    if (u.isAdmin || !u.nickname) return;
    if (u.isParticipating !== false) {
      participating++;
      if (u.status === 'submitted') submitted++;
      if (u.status === 'matched') matched++;
    }
  });
  db.collection('settings').doc('global').set({ participantStats: { participating, submitted, matched } }, { merge: true });
}

function startAdminRealtimeListeners() {
  if (adminListenersActive) return;
  adminListenersActive = true;
  db.collection('users').onSnapshot(snap => { adminUsersSnap = snap; scheduleAdminRender(); updateParticipantCountUI(snap); });
...
function renderAdminFromSnaps(snap, reqSnap) {
  ...
  if (statsBar) {
    const chip = (label, val, color) => `<span style="background:${color}22; color:${color}; border:1px solid ${color}55; border-radius:20px; padding:4px 12px; font-size:0.8rem; font-weight:700; white-space:nowrap;">${label} <b>${val}</b></span>`;
    const allU = Object.values(adminUsersData).filter(u => u.nickname && !u.isAdmin);
    const total = allU.length;
    const part = allU.filter(u => u.isParticipating !== false).length;
    const notPart = total - part;
    const waiting = allU.filter(u => u.isParticipating !== false && u.status === 'waiting').length;
    const submitted = allU.filter(u => u.status === 'submitted').length;
    const matched = allU.filter(u => u.status === 'matched').length;
    const held = allU.filter(u => u.status === 'held').length;
    statsBar.innerHTML = chip('전체', total, '#555') + chip('참여', part, '#27ae60') + chip('불참', notPart, '#e74c3c') + chip('대기', waiting, '#95a5a6') + chip('제출', submitted, '#3498db') + chip('매칭', matched, '#e91e8c') + chip('보류', held, '#e67e22');
  }
...
window.setUserNotParticipating = function(uid, nickname) {
  ...
  if (affected.length > 0) msg += `\n\n⚠️ ${nickname}님을 지망으로 선택한 제출자:\n${affected.map(a => a.name).join(', ')}\n\n이 분들의 해당 지망 항목이 제거됩니다.`;
  if (!confirm(msg)) return;
  const batch = db.batch();
  batch.update(db.collection('users').doc(uid), { isParticipating: false });
  affected.forEach(a => {
    const req = requestsData[a.id] || {};
    const updates = {};
    if (req.pref1 === uid) updates.pref1 = null;
    if (req.pref2 === uid) updates.pref2 = null;
    if (req.pref3 === uid) updates.pref3 = null;
    if (Object.keys(updates).length) batch.update(db.collection('requests').doc(a.id), updates);
  });
...
document.getElementById('admin-start-btn').addEventListener('click', () => {
  ...
  db.collection('users').where('status', '==', 'held').get().then(snap => {
    const batch = db.batch();
    batch.set(db.collection('settings').doc('global'), { isMatchingActive: true, resultsPublished: false }, { merge: true });
    snap.forEach(doc => batch.update(doc.ref, { status: 'submitted' }));
    batch.commit().then(() => alert("✅ 매칭이 시작되었습니다!"));
  });
});
